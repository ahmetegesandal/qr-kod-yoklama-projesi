...
export default async function handler(req, res) {
    if (req.method === 'POST') {
        ...
        try {
            // Aynı oturumun daha önce açılıp açılmadığını kontrol et
            const checkQuery = `
                SELECT roll_call_id
                FROM opened_rollcall
                WHERE ders_id = ? AND tarih = ? AND qr_kod_id = ? AND baslangic_saati = ? AND bitis_saati = ?
            `;
            ...
            if (existingRows.length > 0) {
                return res.status(400).json({ error: 'Bu ders için belirtilen oturum zaten başlatılmış.' });
            }
